...
function StorageNode({ data, selected }: NodeProps<Node<StructNodeData>>) {
  const def = NODE_DEFINITIONS[data.type];

  return (
    <div
      className={`
        rounded-xl border-2 px-4 py-3 min-w-45 transition-all duration-200
    ...
    >
      ...
      {data.description && (
        <p className="text-xs text-[#94a3b8] mt-1 ml-6.5">{data.description}</p>
      )}
      {def?.ports?.output && (
        <Handle
          type="source"
